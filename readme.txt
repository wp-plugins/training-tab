=== Plugin Name ===
Contributors: 
Donate link: http://techstudio.co/wordpress/plugins/training-tab
Tags: training
...
2. Upload the `/training-tab/` directory to the `/wp-content/plugins/` directory
...
= 1.2.1 =
* A few more changes to the interface. Cleaned up the markup too.

= 1.2.0 =
* Refreshed the interface and added compatibility for older browsers.

= 1.1.4 =
* Fixed a bug which caused the request forms to email to the hardcoded address even when a manual address was supplied.
...
* Redesigned response messages when changes are made to training records or if a request is sent.
